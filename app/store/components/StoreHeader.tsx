...
import { useStoreIdentity } from "./StoreIdentityContext";

const navigation = [
  { href: "/store/dashboard", label: "الرئيسية" },
  { href: "/store/products", label: "المنتجات" },
  { href: "/store/categories", label: "التصنيفات" },
  { href: "/store/orders", label: "الطلبات" },
  { href: "/store/customers", label: "العملاء" },
  { href: "/store/settings", label: "الإعدادات" },
];

export function StoreHeader() {
  const { userName, store } = useStoreIdentity();
    ...
    <header className="sticky top-0 z-40 border-b border-line bg-surface/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-4">
          <Link href="/store/dashboard" className="shrink-0">
            <div className="font-display text-lg font-bold text-brand">
              دلالتي
            </div>
            <div className="text-[11px] text-ink-soft">إدارة المتجر</div>
          </Link>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden text-left sm:block" dir="rtl">
            <div className="max-w-32 truncate text-xs font-bold text-ink">
              {userName}
            </div>
            <div className="text-[11px] text-ink-soft">صاحب المتجر</div>
          </div>
          <button
            type="button"
