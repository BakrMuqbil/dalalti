...
export function StoreDetailsModal({ open, store, plans, onClose, onRefresh, onRequest }: Props) {
  ...
  return (
    <Modal open={open} onClose={() => { if (!saving) onClose(); }} size="xl" className="bg-surface">
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-line bg-surface/95 px-6 py-5 backdrop-blur">
        ...
        <Button type="button" variant="ghost" size="sm" onClick={onClose} disabled={saving} className="text-ink-soft hover:bg-background">✕</Button>
      </div>
      ...
      <div className="space-y-5 p-6">
          ...
          {loading ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[['المنتجات', details.counts.products], ['التصنيفات', details.counts.categories], ['العملاء', details.counts.customers], ['الطلبات', details.counts.orders]].map(([label, value]) => <div key={String(label)} className="rounded-2xl border border-line bg-surface p-5"><p className="text-sm text-ink-soft">{label}</p><p className="mt-2 font-mono text-2xl font-semibold">{value}</p></div>)}
            </div>
            ...
            <section className="rounded-2xl border border-line bg-surface p-6">
              ...
              <div className="flex flex-wrap gap-3">
                <Button type="button" variant="secondary" size="sm" disabled={saving} onClick={() => action({ action: details.status === 'ACTIVE' ? 'SUSPEND_STORE' : 'ACTIVATE_STORE' })} className="bg-white hover:border-gold">{details.status === 'ACTIVE' ? 'تجميد المتجر' : 'تفعيل المتجر'}</Button>
                {subscription && <Button type="button" variant="secondary" size="sm" disabled={saving} onClick={() => action({ action: canCancel ? 'CANCEL_SUBSCRIPTION' : 'ACTIVATE_SUBSCRIPTION' })} className="bg-white hover:border-gold">{canCancel ? 'إلغاء الاشتراك' : 'تفعيل الاشتراك'}</Button>}
                <div className="flex items-center gap-2 rounded-xl border border-line bg-background p-1"><input value={extendDays} onChange={(e) => setExtendDays(e.target.value.replace(/\D/g, ""))} className="w-20 bg-transparent px-2 py-2 text-sm outline-none" placeholder="30"/><Button type="button" variant="primary" size="sm" disabled={saving || !subscription} onClick={() => action({ action: 'EXTEND_SUBSCRIPTION', days: Number(extendDays) }, 'تم تمديد الاشتراك')} className="px-3 py-2 text-xs">تمديد يومًا</Button></div>
              </div>
            </section>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <section className="rounded-2xl border border-line bg-surface p-6">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  {[['اسم المتجر','name'],['الرابط','slug'],['هاتف المتجر','phone'],['اسم المالك','ownerName'],['هاتف المالك','ownerPhone'],['البريد','ownerEmail']].map(([label, key]) => <label key={key} htmlFor={key} className="block"><span className="mb-2 block text-xs font-medium text-ink-soft">{label}</span><input id={key} value={form[key as keyof typeof form]} onChange={(e) => setForm((x) => ({ ...x, [key]: e.target.value }))} className="w-full rounded-xl border border-line bg-background px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>)}
                  <label htmlFor="store-description" className="block sm:col-span-2"><span className="mb-2 block text-xs font-medium text-ink-soft">الوصف</span><textarea id="store-description" value={form.description} onChange={(e) => setForm((x) => ({ ...x, description: e.target.value }))} rows={3} className="w-full rounded-xl border border-line bg-background px-3 py-2.5 text-sm outline-none focus:border-gold" /></label>
                  <label htmlFor="store-new-password" className="block sm:col-span-2"><span className="mb-2 block text-xs font-medium text-ink-soft">كلمة مرور جديدة (اختياري)</span><input id="store-new-password" type="password" minLength={8} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full rounded-xl border border-line bg-background px-3 py-2.5 text-sm outline-none focus:border-gold" placeholder="8 أحرف على الأقل" /></label>
                </div>
                <Button type="button" variant="primary" size="md" disabled={saving} onClick={save} className="mt-5 hover:bg-brand-dark">حفظ التعديلات</Button>
              </section>
