...
export function OrdersList({ storeSlug, storeName }: Props) {
  ...
  useEffect(() => {
    const phone = sessionStorage.getItem(`dalalti-account-${storeSlug}`);
    if (!phone) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-24">
        {" "}
        <Spinner size="lg" label="جاري تحميل الطلبات..." />{" "}
      </div>
    );
  }
  return (
    <div className="mx-auto w-full max-w-2xl">
      ...
      {orders.length === 0 ? (
        <EmptyState
          icon={<ReceiptIcon className="h-12 w-12 text-ink-soft" />}
          title="لا توجد طلبات"
          description="لم تقومي بأي طلبات بعد. تصفحي المتجر واختي منتجاتك المفضلة."
        >
          {" "}
          <Link href={`/${storeSlug}`}>
            {" "}
