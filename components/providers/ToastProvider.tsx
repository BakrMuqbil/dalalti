...
// دالة توليد الأصوات برمجياً (Web Audio API)
// دالة تشغيل الملفات الصوتية الحقيقية لضمان المطابقة 100%
const playToastSound = (type: ToastType) => {
  try {
    if (type === "success") {
      const audio = new Audio("/sounds/success.m4a");
      audio.volume = 0.9; // التحكم بمستوى الصوت
      audio.play().catch(() => {});
    } else if (type === "error") {
      // نغمة الفشل برمجياً أو يمكنك إضافة ملف error.mp3 بنفس الطريقة
    ...
    // يتجاهل الأخطاء في البيئات التي تمنع التشغيل التلقائي
  }
};
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-black/5 dark:bg-white/10">
        <div
          className={`h-full ${current.progress} animate-toast-progress`}
        />
      </div>
    </div>
  );
